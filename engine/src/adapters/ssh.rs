use crate::adapters::traits::{Executor, Uploader};
use crate::adapters::{ExecutorError, UploaderError};
use core::fmt;
use log::trace;
use log::{debug, error};
...
impl Ssh {
    ...
    pub fn connect_password(
        ...
        debug!(
            "Connecting to Ssh client {} with {}@{}",
            &address, &username, password
        );
        let tcp = TcpStream::connect(address).map_err(|err| SshError::Tcp(err.to_string()))?;
    ...
    pub fn connect_ssh<T: AsRef<Path>>(
        &mut self,
        host: &String,
        port: u16,
        username: &String,
        ...
        let address: String = format!("{host}:{port}");
        debug!(
            "Connecting to Ssh client {} with {} and ssh_key",
            &address, &username
        );
        let tcp = TcpStream::connect(address).map_err(|err| SshError::Tcp(err.to_string()))?;
        ...
        self.session
            .userauth_pubkey_file(username, None, Path::new(ssh_key.as_ref()), None)
            .map_err(|err| SshError::Authentication(err.to_string()))?;
...
impl Uploader for Ssh {
    ...
    fn upload_file(&self, file: &Path, location: &Path) -> Result<(), UploaderError> {
        ...
        let remote_file: Result<Channel, ssh2::Error> =
            self.session
                .scp_send(location, 0o644, fs::metadata(file).unwrap().len(), None);
